...
**Placeholder replacement is generic.** `apply_pending` drops any sibling at the same source whose `replace_with.matches(this_def)` is true — a swap row evicts its placeholder regardless of category. The replacement row records the placeholder's name in `Feature.replaces: Option<String>` (set in `cascade`); `detect_replacement` fast-paths off this field, falling back to category/prereq heuristic for legacy rows. All swaps — feat replacements, subclass picks, species/background — flow through this one mechanism plus prerequisite filters; no separate identity-slot commit path.

**⚠ THERE ARE NO MARKER FEATURES.** `FeatureCategory::System(_)` is just a category tag; it does NOT make a feature canonical or special. ANY feature can mutate `identity.{species,background,classes,...}` via assign expressions (`SPECIES.X = 1`, `CLASS.X.LEVEL = 1`, etc.). Code must NEVER rely on the presence of a "System(Class)" / "System(Background)" feature to determine identity state — `identity.*` is the source of truth, populated by whatever feature's assigns ran. The previous "markers are canonical, identity is denormalized cache" framing was wrong and led to bugs (e.g. rebuild reading stale identity instead of the would-be-applied marker name).
...
**FeaturesView (`registry.rs`).** Zero-allocation view over natural features index + runtime-synthesized features tagged with `FeatureCategory::System(_)` for picker filtering. Natural takes precedence on key collision. Obtained via `registry.with_features_index{,_untracked}(|view| ...)`. The `System(_)` category is just a filter target for `ReplaceWith::Category(System(_))` — it does NOT make these features canonical.

**Rebuild (`apply/plan.rs` + `apply/rebuild.rs`).** `identity` (species/background/classes) is the source of truth for what the character is — populated by whatever feature's assigns ran during apply. `level_up_plan(identity, features, registry)` returns a canonical `Vec<PendingFeature>` for `build_clean` to walk. Two strategies (NEITHER relies on "marker" features being present):

- `plan_from_markers(identity, features)` — historical path that scans `features.list` for `FeatureCategory::System(IdentitySlot::*)`-tagged rows to recover level-up history. The name is misleading: these rows aren't "markers" in any privileged sense, just feature rows with a category tag. Returns `None` if rows don't cover the identity.
- `plan_from_interleaving(identity, features, registry)` — fallback: walks `identity.classes` round-robin honoring multiclass prereqs against a probe character. **Identity drives the plan, not features.**
