...
**Edit-flow base (feature_row.rs).** Edit modal uses `build_clean(&truncated_clone)` for its pre-edit cascade base — clone the live character, truncate at the edited feature, run the standard rebuild. Single code path for both rebuild and edit-base; no separate `build_cascade_base_before` helper. The pencil shows for any row with interactive inputs **or** `replaces.is_some()`; in the swap case the modal opens for the placeholder (e.g. ASI) with the picker pre-set to the current swap (e.g. Lucky) and Lucky's stored inputs prefilling per-expr. `edit_inputs_modal(store, registry, placeholder_name, source, base, current_name: Option<String>)` — pass `None` for `current_name` for non-swap edits. The submit closure delegates to the pure `compute_edit_outcome` helper (matrix of rename / replaces / dirty rules, unit-tested) and applies its `EditOutcome` to the store. `applied = false` triggers the `BuildReplayHint` "Rebuild" banner (`build_hints.rs:60`); user clicks Rebuild → cascade reruns, swap-row recorded `replaces` drives `detect_replacement` so the new pick lands at the same slot.
