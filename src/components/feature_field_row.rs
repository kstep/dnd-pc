...
#[component]
pub fn FeatureFieldRow(feature_name: StoredValue<String>, field_idx: usize) -> impl IntoView {
    ...
    move || -> Option<_> {
        let field = feature_name.with_value(|key| {
            store
                ...
                .read()
                .get(key)
                .and_then(|e| e.fields.get(field_idx).cloned())
        })?;
        ...
        match &field.value {
            FeatureValue::Die { die, used } => {
                let label = format!("{} {die}", field.label());
                let used_val = used.to_string();
                let max = die.amount;
                Some(EitherOf5::A(view! {
                    <SlotBox label=label>
                        <input
                            type="number"
                            min="0"
