            ...
            <div class="session-section session-section-stats" id="session-stats">
                ...
                <div class="session-core-stats">
                        ...
                        <div class="session-hp-controls">
                            <div class="session-hp-damage">
                                <input
                                    ...
                                    min="1"
                                    required
                                    class="session-damage-input"
                                    node_ref=damage_input
                                />
                                ...
                                <Show when=move || show_damage_picker.get()>
                                    <div class="cast-slot-picker">
                                        <button
                                            class="cast-slot-pill natural-level"
                                            title=move_tr!("damage")
                                        ...
                                        {modifiers
                                            ...
                                            .map(|&damage_type| {
                                                ...
                                                view! {
                                                    ...
                                                    >
                                                        <Icon name=damage_type.icon_name() />
                                                    </button>
                                                }
                                            })
                                            .collect_view()}
                            ...
                            <div class="session-hp-value">
                                ...
                                <input
                                    type="number"
                                    min="0"
                                    class="session-hp-temp-input"
                                    prop:value=move || combat.hp_temp().get()
                                ...
                                /> ")"
                                <span class="session-hp-max">"/ " {move || eff.hp_max()}</span>
                            </div>
                        </div>
                    </div>
                    // -- Inspiration toggle --
                    <div class="session-stat-box session-inspiration-box">
                        ...
                        >
                            {move || {
                                if combat.inspiration().get() { "\u{2605}" } else { "\u{2606}" }
                            }}
                        </button>
                    </div>
                </div>
                        ...
                        <div class="death-save-row">
                            ...
                            <div class="death-save-boxes">
                                {(0u8..3)
                                    .map(|i| {
                                        let checked = move || {
                                            combat.death_save_successes().get() > i
                                        };
                                        view! {
                                            <button
                        ...
                        <div class="death-save-row">
                            ...
                            <div class="death-save-boxes">
                                {(0u8..3)
                                    .map(|i| {
                                        let checked = move || {
                                            combat.death_save_failures().get() > i
                                        };
                                        view! {
                                            <button
                                                class="death-save-box"
                ...
                <div class="slot-box-list">
                    {Ability::iter()
                        .map(|ability| {
                            let tr_key = ability.tr_abbr_key();
                            let label = Signal::derive(move || i18n.tr(tr_key));
                            let proficient = Signal::derive(move || {
                                store.read().proficient_with(ability)
                            });
                            view! {
                ...
                <div class="slot-box-list">
                    {Skill::iter()
                        .map(|skill| {
                            ...
                            let label = Signal::derive(move || i18n.tr(tr_key));
                            let proficient = Signal::derive(move || {
                                store.read().skills.get(skill).is_proficient()
                            });
                            view! {
                                <StatBox label=label highlighted=proficient>
                                    <span class="stat-highlight">
