...
/// - **Placeholder key** (`(ASI, L8)`): when the user (or a recovered
///   prior decision) replaces this slot, `replacement = Some(name)`. Own
///   `inputs` are unused — they belong to the resolved feature, not the
///   placeholder.
